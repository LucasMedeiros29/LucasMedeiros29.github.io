...
'''
Este script é voltado para a aplicação do projeto, onde os pesos do modelo já treinado serão utilizados, bem como
outras funções auxiliares para auxiliar na detecção da imagem do objeto, para detectar e classificar o objeto em
umas das 4 classes: Normal (Indica lixo comum), Low (Indica lixo hospitalar de grau de risco leve), Medium (Indi_
ca lixo hospitalar de grau de risco médio), e High (Indica lixo hospitalar de grau de risco Alto).

As bibliotecas utilizadas foram PyTorch com torchvision (pip install torchvision para instalar),
albumentations (pip install albumentations) e OpenCV (pip install opencv-python).
As bibliotecas OpenCV e albumentations atuam na captura de vídeo e no tratamento dos frames do vídeo para
passar os parâmetros obtidos ao modelo, e auxiliar tanto na detecção do objeto e inserir uma caixa circulando o objeto,
como na classificação doo objeto entre lixo normal e lixo hospitalar com seu grau de risco.
...
# Image Transformations para auxiliar o classificador
...
while True:
    ...
    # Cruza as detecções atuais com os objetos alvo
    ...
    for i in unmatched_detections:
        ...
        with torch.no_grad():
            ...
            #valor para manter o objeto detectado, só muda caso tenha detectado outro no cena > 0.7
            if conf.item() > 0.7:
