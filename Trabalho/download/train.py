#!/usr/bin/env python3
'''
Para o treinamento, após a separação das imagens tratadas pelo prepare_dataset_new.py, é feita uma nova leitura das imagens
para assegurar que as imagens estão tratadas, posteriormente um tratamento aplicando conceitos vistos em aula para treinar o
modelo em diferentes ângulos, ou seja, caso a câmera esteja em um ângulo diferente do que foi registrado no dataset, o modelo
seja capaz de inferir o mesmo resultado. Feito esses passos, é feita a composta dos pixels, com o redimensionamento para
reduzir o tamanho das imagens a normalização e o tensor. Ao chegar no modelo da Convolutional NeuralNetwork (CNN), é utilizado
um modelo já treinado com imagens hospitalares, o ResNet, para auxiliar no treinamento, seguido da definição da rede com a
entrada sendo as features das imagens, camada oculta com função de ativação ReLU, e saída com a classe de risco do lixo caso seja hospitalar ou normal. Também utilizamos CrossEntropyLoss, e o modelo otimizador da rede do tipo Adam.
As bibliotecas são PyTorch (pip install torch) e Albumentations (pip install albumentations)
