...
while True:
    ...
    #grayR= cv2.cvtColor(frameR,cv2.COLOR_BGR2RGB)
    ...
    k = cv2.waitKey(1) 
    if  k == ord('s'):
        cv2.imwrite(output_path+'stereoR/Lucas_Right_%d.png'%i,frameR)
        cv2.imwrite(output_path+'stereoL/Lucas_Left_%d.png'%i,frameL)
